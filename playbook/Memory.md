...
- **Currently working on:** **TASTE PREFERENCES built locally (Session 12, 2026-07-22) — NOT pushed (user said local-only).** The last vision piece. `favorite_cuisines` + `disliked_foods` on `profiles`, surfaced in onboarding + Profile, injection-screened, fed to the plan prompt as best-effort DATA (allergens stay the ONLY enforced constraint), and used to filter dashboard snacks. Gate green: `npm run build`, `test:guardrail` **22/22** (+4 taste-injection checks), `eval` still **100%/100%**. UI visually verified via throwaway `/taste-preview` route (screenshotted, deleted). 4 commits on `feat/taste-preferences` (off `main` `6d58469`). **⚠️ ONE step pending the user: apply `supabase/migrations/0001_taste_preferences.sql` to the live Supabase DB (adds the 2 nullable columns) — until then, saving taste prefs errors. Reads are defensive (`?? []`) so nothing else breaks.** Not pushed; not merged to `main`. **Next after migration+push: Phase 6 (README + submission).**
...
- **Blockers:** none.
- **Git:** remote = `github.com/yarrbakr/Aegis`. `origin/main` = `b03bd1b` (design pass). Local `main` = `6d58469` (design-pass docs). **Unpushed local branch `feat/taste-preferences` (4 commits, off `6d58469`)** — taste prefs, NOT pushed/merged per user's "local-only" instruction. Workflow: branch-per-feature, commit everything, ask before every push. No new deps.
...
## 🔨 In progress
- **Taste preferences on `feat/taste-preferences` (Session 12) — built + verified locally, NOT pushed/merged.** Waiting on the user to (a) apply `supabase/migrations/0001_taste_preferences.sql` to the live DB, then (b) give the go to merge + push.
