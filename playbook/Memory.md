...
- **Currently working on:** **Design pass — dashboard shell BUILT & verified (Session 7b, branch `feat/dashboard-shell`).** App shell (Sidebar + Topbar over an `(app)` route group), redesigned pastel **Dashboard** (stat cards, sage Calories line chart w/ period toggle, macro Report donut, Meal-for-today, guardrail-screened **snack recs**, safety strip), and **Security Console / Meal Plans / Profile** nav pages. `npm run build` green; verified visually via a throwaway `/design-preview` route (mock data), screenshotted, then deleted. Accent locked: **sage = active nav + primary buttons, coral = safety/blocked**. NOT pushed yet. **Next: taste preferences** (needs a `profiles` DB migration + form + prompt wiring) → then push on user's go → Phase 6.
