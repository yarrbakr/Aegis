...
- **Currently working on:** **TASTE PREFERENCES built + hardened locally (Sessions 12–12b, 2026-07-22) — NOT pushed (user said local-only).** The last vision piece. `favorite_cuisines` + `disliked_foods` on `profiles`, surfaced in onboarding + Profile, injection-screened, fed to the plan prompt, and used to filter dashboard snacks. **Migration APPLIED to the live DB this session** (via pooler; both columns verified present). **12b fix:** user tested and a disliked food (mushrooms) still appeared — dislikes were only a soft prompt hint the model half-ignored, so made them **deterministic**: a best-effort TASTE PASS re-rolls any raw meal containing a disliked food BEFORE the allergen guardrail (guardrail stays the authoritative last word; safety never traded for taste). Gate green: `npm run build`, `test:guardrail` **27/27**, `eval` still **100%/100%**. UI visually verified via throwaway `/taste-preview` (screenshotted, deleted). 6 commits on `feat/taste-preferences` (off `main` `6d58469`, head `f6cfb63`). Not pushed/merged. **Next: user tests the dislike fix on local (regenerate a plan) → on their go, merge + push → Phase 6.**
...
- **Blockers:** none.
- **Git:** remote = `github.com/yarrbakr/Aegis`. `origin/main` = `b03bd1b` (design pass). Local `main` = `6d58469` (design-pass docs). **Unpushed local branch `feat/taste-preferences` (6 commits, off `6d58469`, head `f6cfb63`)** — taste prefs + dislike-enforcement fix, NOT pushed/merged per user's "local-only" instruction. Workflow: branch-per-feature, commit everything, ask before every push. No new runtime deps (`pg` was `--no-save`, used once to apply the migration, not in package.json).
...
## 🔨 In progress
- **Taste preferences on `feat/taste-preferences` (Sessions 12–12b) — built + hardened + verified locally, NOT pushed/merged.** Migration is applied. Waiting on the user to (a) test the dislike re-roll on local (regenerate a plan → mushrooms should be gone, allergens still enforced), then (b) give the go to merge + push.

## ⏭️ Next up
1. **Taste preferences — finish the loop (Session 12 built it locally).** (a) user applies `supabase/migrations/0001_taste_preferences.sql` to Supabase (or authorizes the pooler); (b) verify end-to-end on the live/cybergen account (save prefs → generate → prefs reflected, allergens still enforced); (c) on user's go, merge `feat/taste-preferences` → `main` + push (Vercel auto-deploys).
2. **Phase 6 — docs & submission:** `README.md` (decides-then-builds + stack/data-model + locked-decisions table + "what I'd do next" + live link + the eval number **236 meals → 100%/100%** + "built with Claude Code"), finalize Documentary/Prompt/Memory, final deploy check from a fresh browser, email submission (Full Name · Aegis · Live URL · GitHub · 2–3 line description).
...
### 2026-07-22 — Session 12: taste preferences built (local only, not pushed)
- **Attempted:** user said "start work on Taste Preferences, read all the files, don't break the existing app, do everything locally, DON'T PUSH ANYTHING, use it where necessary, log everything." Built the last vision piece end-to-end on `feat/taste-preferences`.
- **Result:** ✅ built + verified locally (nothing pushed).
  - **Data:** `favorite_cuisines` + `disliked_foods` (`text[]`, default `{}`) added to `profiles`. Migration `supabase/migrations/0001_taste_preferences.sql` (additive, idempotent, no new RLS — existing per-row profiles policies cover new columns); `schema.sql` updated for fresh installs; `lib/types.ts` Profile + `onboardingSchema` extended (`COMMON_CUISINES` quick-picks).
  - **Security (the important part):** taste prefs are user free-text that reaches the model, so `screenPreferences` now injection-screens cuisines + dislikes too (drops poisoned entries, logs `injection_detected`). Fed to the prompt as DATA via a `PromptPrefs` override object; system prompt got soft rules 7–8 (lean toward cuisines / avoid dislikes) that **explicitly never override the hard diet+allergen rules — safety always wins over taste**. The deterministic OUTPUT guardrail on real declared allergens is unchanged.
  - **UI:** onboarding gains a "Taste preferences (optional)" fieldset (10 cuisine quick-picks + custom + dislikes input, with safety-hint copy); Profile's "next up" placeholder became a live card (sage cuisine chips + peach dislike chips). All reads guard `?? []` so pages render fine even before the migration runs.
  - **Second surface:** dashboard snacks now also drop disliked snacks (AFTER the safety screen — taste can only hide a safe snack, never surface an unsafe one; falls back to full safe set if dislikes empty it).
- **Verification:** `npm run build` green (11 routes, TS passes); `npm run test:guardrail` **22/22** (added 4 taste-injection checks: clean cuisine/dislike kept, poisoned cuisine dropped+logged); `npm run eval` still **236 meals → 100%/100%**. Couldn't log into local dev (no account creation / passwords); rendered the real taste markup with sample data on a throwaway `/taste-preview` route → screenshotted both the onboarding fieldset and the Profile card → deleted the route + `.next` → rebuilt green.
- **Errors & fixes:** none. (Existing `screenPreferences` test still passes — it destructures a subset of the now-larger return.)
- **Commits (unpushed, `feat/taste-preferences`):** `220e35f` data layer · `d831064` prompt/guardrail/route wiring · `50aa9bc` onboarding+profile UI · `2b76c50` snacks dislikes + tests.
- **⚠️ Pending user action:** apply `0001_taste_preferences.sql` to the live Supabase DB (SQL Editor, or authorize me to via the session pooler) — the ONE step that isn't local. Until then, saving taste prefs will error; everything else works.
- **Next:** user applies the migration → then (on their go) merge `feat/taste-preferences` → `main` + push → Phase 6 (README + submission).

### 2026-07-22 — Session 12b: applied the migration + fixed disliked-food leakage
- **Attempted:** (1) user authorized me to apply the migration ("you do it"); (2) then reported a disliked food (mushrooms) still showed up in a generated plan ("i added mushrooms in the one that I disliked, but it still showed up").
- **Result:** ✅ both done, all local (nothing pushed).
  - **Migration applied:** no `psql`/`pg` locally → `npm install pg --no-save --no-package-lock` (doesn't touch package.json/lockfile), ran a throwaway node script against `SUPABASE_DB_URL` (session pooler, SSL). `alter table ... add column if not exists` → verified via `information_schema`: `favorite_cuisines` + `disliked_foods` both present (`ARRAY`, default `'{}'`). Deleted the script; nothing sensitive printed.
  - **Root cause of the leak:** dislikes were only a **soft prompt hint** (rule 8) and the model half-ignored it (the meal's own description literally said "…replaced with grilled eggplant" while the name kept "Mushrooms"). Data path was correct — pure LLM unreliability.
  - **Fix (the Aegis way — don't trust the model, verify in code):** `lib/taste.ts` `findDislikedTerm` (deterministic substring match, plural-forgiving). New **best-effort TASTE PASS** in the route runs on the RAW meals **before** the allergen guardrail: re-rolls any meal containing a disliked food (bounded ≤2/meal, ≤6/plan; `buildDislikeRegenMessages` + a taste-specific system prompt that still enforces allergens+diet), accepting only a replacement that actually clears the dislike. Runs first so the **guardrail stays the authoritative last word** and its audit log names the final meals. `snacks.ts` reuses the shared matcher.
  - **Safety unchanged:** the deterministic allergen guardrail + eval are untouched; a taste re-roll that (re)introduced an allergen is still caught + regenerated by enforceSafety. **Safety is never traded for taste.**
- **Verification:** `npm run build` green; `test:guardrail` **27/27** (+5 taste-matcher checks incl. the exact mushrooms case); `eval` still **236 → 100%/100%**. Couldn't do a live end-to-end (no login) — left that for the user to test on local; added a `[generate-plan] taste pass: re-rolled N meal(s)…` server log so the re-roll is observable.
- **Commits (unpushed):** `f6cfb63` (`fix(taste): deterministically enforce disliked foods`). Branch head now `f6cfb63` (6 commits).
- **Next:** user regenerates a plan on local to confirm mushrooms are gone (allergens still enforced) → on their go, merge + push → Phase 6.
